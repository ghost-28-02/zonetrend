...
Requires processed data to already exist (run data_pipeline.py first).

Runs:
    1. zone_detector — detect RBD/DBR supply and demand zones

The active symbol is set in config/config.yaml (data.symbol).
Zone detection parameters are in config/zone_config.yaml.
Override the symbol on the command line with --symbol.
...
Output
------
    data/zones/<SYMBOL>_zones.csv  — detected RBD/DBR zones
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ZoneTrend — zone detection pipeline: detect RBD/DBR zones"
    )
    parser.add_argument(
        "--symbol",
    ...
    ok = run_detection(symbol, main_cfg, zone_cfg, logger)
    if not ok:
