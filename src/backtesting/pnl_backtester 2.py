...
def simulate_zone_trade_pnl(
    ...
    trail_atr_mult  : ATR multiplier for trailing stop (default 0.0 = disabled).
                      ...
                      HOW IT WORKS:
                        Phase 1 — trade is open, price hasn't reached target yet:
                          Stop   : fixed at zone distal (thesis invalidation)
                          Target : fixed at rr_ratio × risk (e.g. 2R)
                          → Same behaviour as without trailing stop

                        Phase 2 — price HITS the rr_ratio target:
                          Instead of closing, the trail ACTIVATES from here.
                          Stop   : now trails (trail_atr_mult × ATR) behind the
                                   best price seen since activation — only moves
                                   in the favorable direction, never widens.
                          Target : removed — trade runs freely beyond 2R.
                          Exit   : only when trailing stop is hit.

                      RESULT:
                        - Losses are IDENTICAL to no-trail (zone distal stop unchanged)
                        - Wins are BETTER — minimum 2R guaranteed (trail starts there),
                          maximum is wherever the move extends to.
                        - A trade that would exit at 2R now might exit at 2.8R, 3.5R, 4R
                          depending on how far price runs after the target.

                      Example (trail_atr_mult=1.5, rr_ratio=2.0):
                        Entry ₹100, Stop ₹90 (risk=10), Target ₹120 (2R)
                        Price hits ₹120 → trail activates, trail_dist = 1.5 × ATR
                        If ATR = 8 → trail stop = 120 - 12 = ₹108
                        Price runs to ₹130 → trail = 130 - 12 = ₹118
                        Price reverses, hits ₹118 → exit at ₹118 = +1.8R WAIT...
                        Actually: (118-100)/10 = 1.8R? No — (118-100)/10 = 1.8R.
                        But trail_stop won't go below ₹108 which is already > stop.
                        So minimum exit when trail is active = wherever stop was when
                        target was first hit = target - trail_dist = 120 - 12 = 108 = 0.8R??

                      IMPORTANT NOTE on ATR vs risk sizing:
                        If trail_dist (atr_mult × ATR) > initial_risk, the trail stop
                        at target activation may be below target — meaning you could
                        exit below 2R on a reversal right after hitting target.
                        To guarantee a floor above 2R, use a tighter trail_dist:
                          trail_dist < risk_per_share
                        e.g. if risk_per_share=150pts and ATR=100pts, atr_mult=1.5
                        → trail_dist=150 = risk_per_share → floor exactly at 2R.
                        Use atr_mult=1.0–1.2 for a safer floor.
    ...
    # ── Trailing stop state ───────────────────────────────────────────────────
    #
    # PHASE 1 — before target is hit:
    #   Stop   = fixed zone distal  (thesis invalidation — unchanged)
    #   Target = fixed rr_ratio × risk (e.g. 2R)
    #   Exit   = stop hit (loss) OR target hit (starts phase 2 if trail ON,
    #            or closes as normal win if trail OFF)
    #
    # PHASE 2 — after target is hit AND trail is ON:
    #   Stop   = ATR-based trail following best price  (only tightens, never widens)
    #   Target = removed — trade runs freely beyond 2R
    #   Exit   = trailing stop hit  →  outcome = "trail_win"
    #
    # KEY GUARANTEE: losses are 100% identical to no-trail because the zone
    # distal stop is untouched in Phase 1.  Wins are only extended, never cut.

    use_trail    = trail_atr_mult > 0 and "ATR" in price_df.columns
    trail_active = False        # True once price hits target_price with trail ON
    active_stop  = stop_price   # starts at zone distal; tightens when trail active
    best_price   = entry_price  # best favorable price seen since trail activated
    ...
    for i, row in post_entry.iterrows():
        ...
        # ── Phase 1: check target ─────────────────────────────────────────
        if not trail_active:
            if zone_type == "demand":
                target_reached = high >= target_price
                stop_hit       = low  <= active_stop
            else:
                target_reached = low  <= target_price
                stop_hit       = high >= active_stop

            if stop_hit and not target_reached:
                # Clean loss — zone thesis invalidated
                outcome    = "loss"
            ...
            if target_reached:
                if not use_trail:
                    # Trail disabled: close as normal win at target
                    outcome    = "win"
                    exit_price = target_price
                    exit_date  = row["Date"]
                    break
                else:
                    # Trail enabled: activate trail from target level, keep riding
                    trail_active = True
                    best_price   = high if zone_type == "demand" else low
                    # Immediately set trail stop from this candle's ATR
                    ...
                    if atr > 0:
                        ...
                        if zone_type == "demand":
                            ...
                            candidate = best_price + trail_dist
                        # Trail stop must be at least at zone distal (never widen)
                        if zone_type == "demand" and candidate > active_stop:
                            active_stop = candidate
                        elif zone_type == "supply" and candidate < active_stop:
                            active_stop = candidate
                    # Do NOT break — check if trail stop was also hit on this candle
                    ...
                    if zone_type == "supply" and high >= active_stop:
                        ...
                        exit_date  = row["Date"]
                        break
                    # Still alive — continue to next candle in trail mode
                    hold_candles += 1
                    continue
...
class PnLBacktester:
    ...
    trail_atr_mult    : ATR multiplier for trailing stop (default 0.0 = disabled).
                        When > 0, stop trails (trail_atr_mult × ATR) behind the best
                        price seen since trail_trigger_r profit is reached.
                        0.0 = use fixed zone distal stop (original behaviour).
    trail_trigger_r   : R-profit level at which trailing activates (default 1.0).
                        Trade must be +1R in profit before the trail kicks in.
                        Only relevant when trail_atr_mult > 0.
    ...
    def run(
        ...
        trail_str   = (
            f"{self.trail_atr_mult}×ATR (activates at 2R target hit)"
    ...
    def print_report(self, metrics: dict, symbol: str) -> str:
        ...
        lines = [
            ...
            f"  Closed (W+L+Trail)   :  {metrics['closed_trades']}",
            f"    Wins               :  {metrics['wins']}",
            f"    Losses (zone stop) :  {metrics['losses']}",
            f"    Trail stop exits   :  {metrics['trail_stop_exits']}",
            ...
            f"  Avg Trail Stop (₹)   :  ₹{metrics['avg_trail_stop_inr']:>+10,.2f}",
